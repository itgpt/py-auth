...
def get_auth_background_executor() -> ThreadPoolExecutor:
    ...
    with _background_auth_lock:
        if _background_auth_executor is None:
            ...
            _background_auth_executor = ThreadPoolExecutor(
                max_workers=max(4, n), thread_name_prefix="py_auth_client"
            )
        return _background_auth_executor


def shutdown_auth_background_executor(
...
class AuthCache:
    ...
    def save_cache(
        ...
        *,
        ...
        heartbeat_times: Optional[int] = None,
        device_info_snapshot: Optional[dict[str, Any]] = None,
    ) -> bool:
        try:
            now = time.time()
            ...
            if not authorized:
                ...
                if (
                    device_info_snapshot is not None
                    and isinstance(device_info_snapshot, dict)
                    and (len(device_info_snapshot) > 0)
                ):
                    sub[BUNDLE_PRODUCT_DEVICE_INFO_SNAPSHOT_KEY] = copy.deepcopy(
...
class AuthClient:
    ...
    def __init__(
        ...
        if device_id:
            ...
            _loaded_pid = load_persisted_device_id(
                self.server_url, software_name, base_dir=self._storage_base
            )
        need_facts_for_new_id = (
            not device_id
            and _loaded_pid is not _PERSISTED_DEVICE_ID_NOT_PREFETCHED
            and (not _loaded_pid)
        )
        ...
        self.device_id = build_device_id(
            ...
            software_name,
            base_dir=self._storage_base,
            persisted_device_id=(
                _loaded_pid if not device_id else _PERSISTED_DEVICE_ID_NOT_PREFETCHED
            ),
        )
        try:
        ...
        self.device_info["sdk"] = {
            "language": "python",
            "sdk_name": "py_auth_client",
        ...
        if self._device_info_deferred:
            if need_facts_for_new_id and _prefetch_fut is not None:
                ...
                self._facts_prefetch_future = self._facts_prefetch_executor.submit(
                    collect_device_facts
                )
    ...
    def _post_heartbeat(
        self, di: dict[str, Any], heartbeat_times: int
    ) -> dict[str, Any]:
        import requests
        ...
        try:
            ...
            if response.status_code == 200:
                decrypted = self._decrypt_data(
                    response.json().get("encrypted_data", "")
                )
                if decrypted:
                    self._log_debug(
                        f"在线订阅成功，authorized={decrypted.get('authorized')}"
                    )
                    return {
            ...
            error_msg = (
                response.json().get("detail", f"服务器错误: {response.status_code}")
                if response.status_code == 403
                else f"服务器错误: {response.status_code}"
            )
            self._log_debug(
                f"在线订阅失败，status={response.status_code}, message={error_msg}"
            )
            return {
                "authorized": False,
                "message": error_msg,
    ...
    def _check_online_worker(self, heartbeat_times: int) -> dict[str, Any]:
        ...
        _need_pub = self._device_info_deferred or _device_info_lacks_nonblank_public_ip(
            self.device_info
        )
        try:
    ...
    def _check_online_fast(self, heartbeat_times: int) -> dict[str, Any]:
        ...
        try:
            ...
            try:
                ...
                self._log_debug(
                    f"轻量在线订阅超出时限 {_ONLINE_CHECK_FAST_WALL_SEC:g}s"
                )
                return {
                    "authorized": False,
                    "message": f"连接失败: 轻量请求超时（{_ONLINE_CHECK_FAST_WALL_SEC:g}s）",
                    "success": False,
                    "from_cache": False,
                }
        finally:
    ...
    def _check_online(self, heartbeat_times: int) -> dict[str, Any]:
        ...
        _wall = _online_check_effective_wall_sec(
            self._device_info_deferred
            or _device_info_lacks_nonblank_public_ip(self.device_info)
        )
        executor = ThreadPoolExecutor(max_workers=1)
        try:
            ...
            try:
                ...
                self._log_debug(
                    f"在线订阅超出总时限 {_wall}s（含 device_info 推迟时的全量采集 + 公网 IP + 心跳；Windows 等对无服务地址的 TCP 重传可能绕过单阶段 timeout）"
                )
                return {
                    "authorized": False,
                    "message": f"连接失败: 请求超时（在线阶段墙钟上限 {_wall:g}s）",
                    "success": False,
                    "from_cache": False,
                }
        finally:
    ...
    def _write_check_cache_retries(
        self,
        online_result: dict[str, Any],
        heartbeat_times_if_authorized: Optional[int],
    ) -> bool:
        hb = heartbeat_times_if_authorized if online_result.get("authorized") else None
        snap = (
            copy.deepcopy(self.device_info) if online_result.get("authorized") else None
        )
        saved = False
        for _attempt in range(3):
            if self.cache.save_cache(
                online_result["authorized"],
                "",
                heartbeat_times=hb,
                device_info_snapshot=snap,
            ):
                saved = True
                break
            time.sleep(0.05)
    ...
    def check_authorization_progressive(
        ...
        if r_fast.get("success"):
            if r_fast.get("authorized"):
                ...
                if r_full.get("success"):
                    self._log_debug("在线订阅成功，更新缓存")
                    self._write_check_cache_retries(
                        r_full, (next_hb + 1) if r_full.get("authorized") else None
                    )
                    return r_full
                ...
                if gc and self.cache.is_cache_valid():
                    ...
                    self._log_debug(
                        f"补全心跳失败，沿用轻量结果，订阅剩余时间: {remaining}"
                    )
                    return {
                        "authorized": True,
        ...
        if online_result["success"]:
            self._log_debug("在线订阅成功，更新缓存")
            self._write_check_cache_retries(
                online_result, next_hb if online_result["authorized"] else None
            )
        ...
        if cache_valid:
            ...
            self._log_debug(
                f"在线订阅失败，但缓存有效，使用缓存结果: {online_result.get('message')}，订阅剩余时间: {remaining}"
            )
            return {
                "authorized": True,
        ...
        if cache_data:
            ...
            remaining = self._format_remaining_time(cached_at)
            self._log_debug(
                f"在线订阅失败，缓存已过期，返回失败结果: {online_result.get('message')}，订阅剩余时间: {remaining}"
            )
        else:
            self._log_debug(
                f"在线订阅失败，返回失败结果: {online_result.get('message')}"
            )
        return online_result
    ...
    def require_authorization(
        self, raise_exception: bool = True, force_online: bool = False
    ) -> bool:
    ...
    def submit_check_authorization(self, force_online: bool = False) -> Future:
        return get_auth_background_executor().submit(
            self.check_authorization, force_online
        )

    def submit_check_authorization_progressive(
        self, force_online: bool = False
    ) -> Future:
        return get_auth_background_executor().submit(
            self.check_authorization_progressive, force_online
        )

    def submit_require_authorization(
        self, *, raise_exception: bool = True, force_online: bool = False
    ) -> Future:
        return get_auth_background_executor().submit(
            self.require_authorization, raise_exception, force_online
        )
    ...
    def get_authorization_info(self) -> dict[str, Any]:
        ...
        if cache:
            ...
            if cached_at > 0:
                info["cached_at_readable"] = time.strftime(
    ...
    def get_cache_info(self) -> Optional[dict[str, Any]]:
        ...
        return {
            ...
            "cache_age_days": (now - cached_at) / _SECONDS_PER_DAY,
...
class AuthorizationError(Exception):
    ...
    def is_unauthorized(self) -> bool:
        if self.result:
            return not self.result.get("authorized", False) and self.result.get(
                "success", False
            )
