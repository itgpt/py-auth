...
import { buildDeviceId, collectDeviceInfo } from "./device";
...
export class AuthClient {
  ...
  private formatRemainingTime(cachedAtSeconds: number): string {
    if (!cachedAtSeconds || cachedAtSeconds <= 0 || !this.cache) return "未知";
  ...
  private async checkOnline(): Promise<AuthResult> {
    try {
  ...
  async requireAuthorization(): Promise<boolean> {
